...
@Component({
  selector: "app-fail-details",
  templateUrl: "./fail-details.component.html",
  styleUrls: ["./fail-details.component.scss"],
})
export class FailDetailsComponent implements OnInit, OnDestroy {
  // Chart
  chart: any;
  ...
  ngOnInit() {
    ...
    this.FailData.getOne(this.proid).subscribe((res) => {
      this.listFail = res;
      // this.tableRows = [...res];
  ...
  addFailDetails() {
    ...
    this.FailData.create(this.addFailDetailsForm.value).subscribe(
      () => {
        // Success
  ...
  editFailDetails() {
    // console.log("qqqq");
    this.loadingBar.start();
    this.FailData.update(
      this.editFailDetailsForm.value.id,
      this.editFailDetailsForm.value
    ).subscribe(
      () => {
        // Success
        // this.isLoading = false
